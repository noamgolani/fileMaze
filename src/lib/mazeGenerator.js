...
function createClues(dir, rooms, depth) {
  ...
  if (Math.random() < 0.3) fs.writeFileSync(`${dir}/chest-2.nosj`, 'I`m a bad clue');
  if (Math.random() < 0.3) {
    fs.writeFileSync(
      `${dir}/chest-3.json`,
      JSON.stringify({
